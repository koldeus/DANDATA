...
export default function GraphPreview({ graphique, metadonnees, theme }) {
  ...
  useEffect(() => {
    if (!graphique.metadonnees || graphique.variables.length === 0) {
      setChartData([]);
      setVariablesObj([]);
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        // Find indices for ONLY selected variables
        ...
        if (hasTextVariables) {
          ...
          const data = lines.slice(1, 11).map((line, index) => {
            ...
            // Utiliser la variable d'identification si elle existe
    ...
    fetchData();
  }, [graphique.metadonnees, JSON.stringify(graphique.variables)]);
  ...
  return (
    <div className={`chart-preview ${theme}_Graph ${theme}_Border`}>
      ...
      <small className="chart-preview-note">
        * Preview limitée aux 10 premières lignes • {variablesObj.length}{" "}
        variable(s) sélectionnée(s)
